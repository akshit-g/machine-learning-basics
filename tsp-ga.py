...
best_path = 10000  # any large value to be updated later

nodes = [0, 1, 2, 3, 4]

# adjacency matrix for our nodes
weights = [[0, 4, 4, 7, 3],
             [4, 0, 2, 3, 5],
             [4, 2, 0, 2, 3],
             [7, 3, 2, 0, 6],
...
# calculates weight between 2 nodes
def calc_weight(city1, city2):
    return weights[city1][city2]  # ord('A')=65
...
def calc_route_length():
    for i in range(population_size):
        ...
        for j in range(1, len(nodes)):
            route_l = route_l + calc_weight(population[i][j - 1], population[i][j])
        # route_l = route_l + calc_weight(population[i][len(nodes)-1], population[i][1]) calculate weight from last to first
...
# swap with a probability 2 nodes in a route
def swap_mutation(ind):
    picks = random.sample(range(len(nodes)), 2)
